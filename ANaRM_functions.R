#A collection of functions called by the main program. John R Wallbank April 2022
...
exportTIFF <- function(datMat,rasterTemplate,nameStr,FN,dePad,dType = 'FLT4S'){#
  printp("EXPORTING : ",FN)
  if(dePad){
...
mkGriddedLakeDFv2 <- function(lakeMP,ccarMP,outfIndx){
  ...
  while( length(allLakeCells)!=0 ){
    ...
    lakesCCAR = c( lakesCCAR , ccarMP[lakeout])
  }
